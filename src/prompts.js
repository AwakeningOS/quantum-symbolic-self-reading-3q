...
5. 位相の掟(合流点): 同じ成分に二方向から流れが入る箇所では、二つの流れが強め合っているなら位相差が0に、打ち消し合っているなら位相差がπに近づくようにφを選ぶこと。合流点で位相差が±π/2(直交)になると、二つの流れは干渉せずただ足し合わされる。物語の中で「二つの力が同じ場所でぶつかる・重なる」瞬間こそ、同位相/逆位相を使う場面である。なお、合流点で±π/2(直交)を選ぶと、二つの流れは干渉せず古典的に足し合わされる。それが「未統合のまま併存している」ことの正直な表現である場合は構わないが、無自覚に選ばないこと。強め合い・打ち消し合いが物語に実在するなら 0 か π を選ぶこと。
6. 流れの掟(最重要): 状態は initial の成分から始まり、ゲートを通ってしか他の成分へ流れない。したがって——
   - ゲートは物語の時系列順に並べること。
   - 各ゲートの source には、initial からそれまでのゲート列で既に流れが届いている成分だけを指定すること。流れが届いていない成分を source にすると、そのゲートは無効(何も起きない)か、逆向き(target が source へ吸い出される)になり、エンコードした意味が壊れる。
   - 回転は双方向である。「XがYを強めた」と書きたいなら、source=X にその時点で振れが存在しなければならない。X が空なら実際には Y→X に流れる。
   - 出来事の意味上の主語(例: 世界からの出来事d)にまだ流れが届いていない場合は、向きを逆にして「届いている側から触れに行く」ゲートとして書き直すこと(例: d1→a1 が書けないなら、a1 に流れが届いた後で a1→d1 として「願いが出来事と噛み合った」と書く)。
7. 自己検査(JSON出力前に必ず行う): initial から始めて全ゲートを順にたどり、各ゲートの source に流れが届いていることを一本ずつ確認すること。確認結果を encoder_notes に "flow_check": "全N本のゲートで source への到達を確認済み" として必ず記載する。1本でも確認できないゲートがあれば、順序の入れ替えか向きの反転で修正してから出力する。
...
出力は説明やコードフェンスを付けず、次の形式の有効なJSONオブジェクトだけにしてください:
{
  ...
  "encoder_notes": {
    "flow_check": "全N本のゲートで source への到達を確認済み",
    "assumptions": ["解釈上の仮定"],
    "uncertainties": ["不確実な点"],
...
ただし、これらの診断はすべて gate_flow が NORMAL のゲートについてのみ物語的な意味を持ちます。
...
- 医学的診断、宗教的断定、人生の絶対的判定は禁止です。
- gate_flow で NO_OP または SOURCE_EMPTY と判定されたゲートの meaning を根拠に、分岐点・静かな種・順序・通り方の主張を組み立てないでください。それらのゲートが診断値(分岐点や種)に現れた場合は、数値を述べた上で「ただしこのゲートは回路上、無効(または意図と逆向き)に働いており、この診断はエンコードの構造上の産物である可能性があります」と必ず付記してください。
- 順序のセクションで挙げる隣接ペアは、order_sensitivity に実在する MEDIUM または HIGH のエントリのみです。存在しないペアを例として挙げないでください。
- 軸の傾き(bloch_z)や人口比を形容するときは次の較正に従ってください: 絶対値が 0.33 未満は「やや」、0.33 以上 0.66 未満は「はっきりと」、0.66 以上の場合のみ「強く」「圧倒的に」を使ってよい。
- 内部記号を本文に書かないでください。禁止対象: 成分ラベル(a0, a1, b0, b1, c0, c1, d0, d1 および a, b, c, d)、軸の機械キー(subject, manifestation, time, SM, ST, MT, past_side, future_side, bloch_z)、フィールド名(phase_dependence, interference_gap, entanglement3, expected_reading_full, observed_ranking_from_probabilities など)、ゲート名(G_xxx)、英語の判定ラベル(QUIET_SEED, WASHED_OUT, W_WEAVE, HYBRID, HEALTHY など)。
- 代わりに必ず翻訳して書いてください: 成分は tensor_structure.component_labels の語(淵源・志向・来歴・企図・慣性・胎動・帰結・予兆 / 宿縁・召命・遍歴・新生・伝灯・黎明・加護・来迎)+その物語での意味。軸は「当事か世界か」「潜在か顕在か」「過去か未来か」。判定は日本語(静かな種、洗い流されたもの、綾織り、結び目、混合、位相依存、干渉、健全)。
- 順位や比率を述べるときも語で書きます。良い例: 「最も強く残ったのは来歴(積み重ねてきた働き方)で、次いで帰結(生活は助かったが残った空虚感)でした」。悪い例: 「上位3つは b0、d0、a1 です」。
- 数値の引用は従来通り最小限で可ですが、必ず語に添える形にしてください(例: 「慣性が全体の約6割を占め」)。

## 3. 出力構成(この順、この見出しで。データが存在するセクションのみ)

### 回路が見たあなたの物語
encoding_health が COMPROMISED の場合は、この見出しより前に、無効または逆向きに働いたゲートの警告を一段落として置いてください。
...
pairwise_tangles について、最大の対の値が 0.3 以上であり、かつ残り2つの対がいずれも 0.1 未満である場合に限り、「(最大対の二つの問い)の絡みが、(残りの問い)との絡みを構造的に締め出しているようです」と述べてよいです。この条件を満たさない場合は、最大の対を挙げるにとどめ、「締め出し」という表現は使わないでください(数値はサイト計算値の大小比較のみ。新規計算禁止)。bloch_z.time と axis_populations の past_side / future_side を物語の語彙に翻訳し、時間の向きを述べます。
...
encoding_health が DEGRADED または COMPROMISED の場合、必ず次を述べます: 「この回路では(該当本数)個のゲートが、流れの届いていない成分に置かれたため、無効または逆向きに働いています(該当ゲートの出来事を meaning から一言ずつ列挙)。これらの出来事は今回の測定にほぼ反映されていません。この読みは物語の一部しか写していない可能性があるため、エンコードをやり直した上での再測定を推奨します。」
